...
export default function MACAgentList() {
  ...
  return (
    <PageLayout title="Agents" rightAction={<span style={{ fontSize: 12, color: '#999' }}>{agents.length} 个 Agent</span>}>
      <DataToolbar
        buttons={<button className="action-btn primary" onClick={() => navigate('/managed-agent/agents/create')}>+ 创建 Agent</button>}
        filters={
              ...
              {filtered.length === 0 ? (
                <tr><td colSpan={9} className="data-table-empty">
                  <div className="empty-state ha-empty">
                    ...
                    <button className="action-btn primary" style={{ marginTop: 12 }} onClick={() => navigate('/managed-agent/agents/create')}>+ 创建 Agent</button>
                  </div>
                </td></tr>
              ) : filtered.map(agent => {
                  ...
                  <tr key={agent.id}>
                    ...
                    <td>
                      <div className="ha-row-actions">
                        ...
                        <button className="ha-delete-btn" onClick={() => setConfirmDelete(agent)}>删除</button>
                      </div>
                    </td>
                  </tr>
                )
        ...
        <div className="ha-card-grid">
          {filtered.length === 0 ? (
            <div className="ha-empty-card-state">
              ...
              <button className="action-btn primary" style={{ marginTop: 12 }} onClick={() => navigate('/managed-agent/agents/create')}>+ 创建</button>
            </div>
          ) : filtered.map(agent => {
            const sc = AGENT_STATUS_COLORS[agent.status] || AGENT_STATUS_COLORS.draft
            ...
            return (
              <div key={agent.id} className="ha-agent-card mac-agent-card-enhanced" onClick={() => navigate(`/managed-agent/agents/${agent.id}`)}>
                ...
                <div className="mac-card-capabilities">
                  <span className="mac-cap-pill" title="工具数">🔧 {toolCount}</span>
                  <span className="mac-cap-pill" title="Skills">🧩 {skillCount}</span>
                  {hasMcp && <span className="mac-cap-pill" title="MCP 已连接">🌐 MCP</span>}
                  <span className="mac-cap-pill" title="模型">🧠 {agent.model?.modelId?.split('-')[0]}</span>
                </div>
                ...
                <div className="ha-card-actions" onClick={e => e.stopPropagation()}>
                  <button onClick={() => navigate(`/managed-agent/agents/${agent.id}`)} title="详情">📋</button>
                  <button onClick={() => navigate('/managed-agent/sessions', { state: { agentId: agent.id } })} title="新建会话">⚡</button>
                  <button onClick={() => { dispatch({ type: 'CREATE_AGENT', payload: { ...agent, name: agent.name + '-clone', description: agent.description + ' (克隆)' } }); showToast('已克隆') }} title="克隆">📋</button>
                  <button className="ha-delete-btn" onClick={() => setConfirmDelete(agent)} title="删除">🗑</button>
                </div>
              </div>
            )
          })}
